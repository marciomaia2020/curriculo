...
function handleNumberClick(event) {
  ...
  if (numberInGame) {
    ...
    var quantityInput = document.getElementById('number-amount');
    var quantity = parseInt(quantityInput.value);

    if (totalNumbers === quantity) {
      return;
    } else {
...
function renderClearSavedGamesButton() {
  ...
  button.addEventListener('click', handleClearSavedGames);
...
function handleSaveGame() {
  ...
  if (state.currentGame.length !== quantity) {
    alert('Você precisa selecionar exatamente ' + quantity + ' números.');
    return;
  }
...
function handleClearSavedGames() {
  state.savedGames = [];
  writeLocalStorage();
  render();
}
...
function randomGame() {
  ...
  if (quantity < 6 || quantity > 20) {
    alert('A quantidade de números deve estar entre 6 e 20.');
    return;
  }

  state.currentGame = [];

  while (state.currentGame.length < quantity) {
